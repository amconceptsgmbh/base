...
export let allowSpeaking = 'speaking';
export let sessionIsFinished = false;
export let currentPersonaAnswer = '';
...
const initialState = {
  ...
  transcript: [],
  activeCards: [],
  speechState: 'idle', // idle speaking
...
export const createScene = createAsyncThunk('sm/createScene', async (_, thunk) => {
  ...
  scene.onMessage = (message) => {
    // console.log("$$$ scene.onMessage got triggered. Hopefully this happens often!!!!");
    // removing this will break smwebsdk eventing, call smwebsdk's message handler
    smwebsdkOnMessage(message);
    //console.log('New sm message received:');
    //console.log(message);
    // console.log('New spechState received');
    // console.log(message.body?.persona?.[1]?.speechState);
    //console.log("body");
    //console.log(message.body);
    // Vielen Dank, dass Sie am Bewerbungsgespraech der A M Concepts GmbH teilgenommen haben. Wir analysieren nun Ihre Leistung und senden Ihnen unsere ausfuehrliche Analyse als Report per Email zu. Bis zum naechsten Mal! Sie werden nun weitergeleitet.
    // Vielen Dank für Ihre Antwort. Wir freuen uns, dass Sie am Bewerbungsgespraech der A M Concepts GmbH teilgenommen haben. Wir analysieren nun Ihre Leistung und senden Ihnen unsere ausfuehrliche Analyse als Report per Email zu. Bis zum naechsten Mal! Sie werden nun weitergeleitet.
    if (message.body?.currentSpeech){
      currentPersonaAnswer = message.body?.currentSpeech;
    }

    if (message.body?.currentSpeech === 'Vielen Dank für Ihre Antwort. Wir freuen uns, dass Sie am Bewerbungsgespraech der A M Concepts GmbH teilgenommen haben. Wir analysieren nun Ihre Leistung und senden Ihnen unsere ausfuehrliche Analyse als Report per Email zu. Bis zum naechsten Mal! Sie werden nun weitergeleitet.') {
      sessionIsFinished = true;
      console.log('Session is finished.');
    }

    if (message.body?.persona?.[1]?.speechState === 'idle') {
    ...
    switch (message.name) {
      ...
      case ('recognizeResults'): {
        ...
        if (output.final === false) {
          console.log(`Text from not final Speech Recognition: ${text}`);
        ...
        if ('persona' in body) {
          ...
          if ('users' in personaState) {
            ...
            if ('emotion' in userState) {
              ...
              // console.log("Rounded emotion is:");
              // console.log(roundedEmotion);
            ...
            if ('activity' in userState) {
              ...
              // console.log("Rounded activity is:");
              // console.log(roundedActivity);
  ...
  try {
    ...
    console.log('Session ID from React code is: ');
...
const smSlice = createSlice({
  ...
  reducers: {
    ...
    setCameraOn: (state, { payload }) => {
      // console.log('Persona conversationGetVariables()'); // can be removed later
      // console.log(persona.conversationGetVariables()); // can be removed later
      // console.log('Persona speech state'); // can be removed later
      // console.log(persona.speechState()); // can be removed later
